...
class ProcessSchedulerApp:
    ...
    def build_gui(self):
        ...
        self.output_canvas = tk.Canvas(self.root, width=1100, height=150, bg="white")
    ...
    def display_timeline(self, timeline):
        ...
        for process, start, end in timeline:
            bar_width = (end - start) * 10  # Scale bar width
            self.output_canvas.create_rectangle(
                x_offset + start * 10, y_offset,
                x_offset + end * 10, y_offset + bar_height,
                fill="skyblue", outline="black"
            )
            self.output_canvas.create_text(
                x_offset + (start + end) * 5, y_offset + bar_height / 2,
                text=process, fill="black"
            )
            # Add start and end times
            self.output_canvas.create_text(x_offset + start * 10, y_offset + bar_height + 10, text=str(start))
            self.output_canvas.create_text(x_offset + end * 10, y_offset + bar_height + 10, text=str(end))
